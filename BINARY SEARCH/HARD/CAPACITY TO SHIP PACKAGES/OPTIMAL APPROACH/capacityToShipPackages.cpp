...
/*
    Time Complexity - 
        O(N * log(sum(weights[]) - max(weights[]) + 1)), where sum(weights[]) = summation of all the weights, max(weights[]) = maximum of all the weights, N = size of the weights array.        
    Space Complexity - 
        O(1)
*/
...
int leastWeightCapacity(vector<int> &weights, int d) {
  ...
  while (low <= high) {
    ...
    if (daysReq(weights, mid) <= d) high = mid - 1;
    else low = mid + 1;
  }
  return low;
